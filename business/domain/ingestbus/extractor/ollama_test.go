...
func TestOllamaExtractText_Success(t *testing.T) {
	...
	ex := newTestExtractor(t, srv.URL, "llama3")
	got, err := ex.ExtractText(context.Background(), "some voice capture text", "", []ContextRef{}, "", 0, nil, nil)
	if err != nil {
...
func TestOllamaExtractText_Transaction(t *testing.T) {
	...
	ex := newTestExtractor(t, srv.URL, "llama3")
	got, err := ex.ExtractText(context.Background(), "AMZN MKTP US*AB1CD2EF3", "", []ContextRef{}, "transaction", 0, nil, nil)
	if err != nil {

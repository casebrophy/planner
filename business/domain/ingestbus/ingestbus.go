...
func (b *Business) processRawInput(ctx context.Context, ri rawinputbus.RawInput, rawContent string) error {
	...
	if matchedContextID != nil && extraction.ContextConfidence > 0 && extraction.ContextConfidence < 0.7 && ri.SourceType != rawinputsource.Transaction {
		...
		if _, err := b.clarificationBus.Upsert(ctx, clarificationbus.NewClarificationItem{
			...
			SubjectType:        "email",
			SubjectID:          email.ID,
			SubjectDescription: fmt.Sprintf("Email: %s (from %s)", parsed.Subject, parsed.FromAddress),
	...
	for _, cr := range clauseResults {
		...
		if unconfirmed && ri.SourceType != rawinputsource.Transaction {
			...
			if _, err := b.clarificationBus.Upsert(ctx, clarificationbus.NewClarificationItem{
				...
				SubjectType:        "raw_input",
				SubjectID:          ri.ID,
				SubjectDescription: fmt.Sprintf("Clause: %s", truncateDesc(cr.clause, 120)),
	...
	for _, cr := range clauseResults {
		for _, ref := range cr.extraction.AmbiguousReferences {
			...
			if _, err := b.clarificationBus.Upsert(ctx, clarificationbus.NewClarificationItem{
				...
				SubjectType:        "raw_input",
				SubjectID:          ri.ID,
				SubjectDescription: fmt.Sprintf("Voice reference: %q", ref.OriginalText),
